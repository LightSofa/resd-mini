...
const open = () => {
  ...
  store.openProxy().then((res: appType.Res) => {
    ...
    if (["darwin", "linux"].includes(store.appInfo.Platform)) {
...
const handleInstall = async () => {
  ...
  if (store.appInfo.Platform === "windows" && res.message.includes("Access is denied")) {
    window.$message?.error(t("index.win_install_tip"))
  } else if (["darwin", "linux"].includes(store.appInfo.Platform)) {
    showPassword.value = true
  }
  return false
}
